const showNowPlaying = (data) => {
    const listMovieElement = document.querySelector("#listNowPlaying");
    ...
    data.forEach(movie => {
        listMovieElement.innerHTML += `
            <style>
                ...
                #listNowPlaying::-webkit-scrollbar {
                    width: 13px;
                    height: 7px;
                }

                #listNowPlaying::-webkit-scrollbar-thumb {
                    background: rgba(90, 90, 90);
                    border-radius : 10px;
                }

                #listNowPlaying::-webkit-scrollbar-track {
                    background: rgba(0, 0, 0, 0.2);
                    border-radius : 10px;
                }

                @media screen and (min-width: 763px){
                    #listNowPlaying:hover{
                        overflow-x:scroll;
                    }
                }
            ...
            <div class="card-box">
                <img class="fan-art-movie detail-info" src="https://image.tmdb.org/t/p/w500/`+ movie.poster_path + `" data-toggle="modal" data-target="#exampleModal" 
                        data-id="${movie.id}" alt="Fan Art">
    ...
    $('#listNowPlaying').on('click', '.detail-info', function () {
